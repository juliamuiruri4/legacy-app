...
test.describe("Assets list — happy path", () => {
  test("renders heading and table column headers", async ({ page }) => {
    ...
    await expect(table.getByRole("columnheader", { name: "Tag" })).toBeVisible();
    await expect(table.getByRole("columnheader", { name: "Type" })).toBeVisible();
    await expect(table.getByRole("columnheader", { name: "Make / Model" })).toBeVisible();
    await expect(table.getByRole("columnheader", { name: "Serial" })).toBeVisible();
